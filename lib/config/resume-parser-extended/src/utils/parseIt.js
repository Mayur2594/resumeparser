...
var parser = {
  ...
  extractText: function(path, type, cbAfterParse) {
    const objParseBoy = new ParseBoy();
    if (type.includes("/")) {
      processing.runBuffer(path, type, (preppedFile, error) => {
        return objParseBoy.extractTextFromFile(preppedFile, parsedResume =>
          cbAfterParse(parsedResume, error)
        );
      });
    } else if (type === "url") {
      ...
      processing.runFile(path, (preppedFile, error) => {
        objParseBoy.extractTextFromFile(preppedFile, parsedResume =>
          cbAfterParse(parsedResume, error)
        );
      });
    }
